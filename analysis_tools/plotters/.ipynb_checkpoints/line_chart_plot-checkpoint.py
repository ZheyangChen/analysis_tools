...
def plot_difference(
    ...
    for dkey, df in data_dict.items():
        ...
        for yc in y_cols:
            ...
            # Group and compute weighted mean/std (take only first 2 returned values)
            stats = tmp.groupby("bin").apply(
                lambda g: tuple(weighted_stats(g["diff"], g["w"])[:2])
            )
            
            stats = stats.reindex(range(len(bins)-1))
            stats = stats.apply(
                lambda x: x[:2] if (isinstance(x, tuple) and len(x) >= 2)
                else (np.nan, np.nan)
            )
            stats_df = pd.DataFrame(list(stats.values), columns=["mean", "std"])
                        
            means = stats_df["mean"].values
            stds  = stats_df["std"].values
            valid = ~np.isnan(means)
            
            # ---- NEW: percentage error option ----
            if percentage:
                # compute weighted mean(true) per bin (use tmp, not df!)
                tmp["true"] = true
                true_stats = (
                    tmp.groupby("bin")["true"]
                       .apply(lambda g: np.average(g, weights=tmp.loc[g.index, "w"]))
                       .reindex(range(len(bins)-1))
                )
            
                true_vals = true_stats.values
                true_vals[true_vals == 0] = np.nan
            
                means = (means / true_vals) * 100.0
            
                ylabel = ylabel or "Percentage Error (%)"
            else:
